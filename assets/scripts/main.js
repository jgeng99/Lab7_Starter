...
async function getRecipes() {
  ...
  let recipesExist = localStorage.getItem('recipes');
  if (recipesExist) {
    return JSON.parse(recipesExist);
  }
  /**************************/
  ...
  return new Promise((resolve, reject) => {
    ...
    RECIPE_URLS.forEach(async (url) => {
      ...
      try {
        // A6. For each URL in that array, fetch the URL NOTE: Fetches are ASYNCHRONOUS, meaning that
